...
import type { Action, AuthUser, PolicyDecision, PolicyFn, ResourceAttributes } from "./types"
...
export const POLICIES: Partial<Record<Action, PolicyFn>> = {
    ...
    "adjustments:create": (user, attrs) => {
        ...
        if (attrs?.product) {
            ...
            if (isArchived) {
                return {
                    allowed: false,
                    reason: "Unauthorized. Cannot perform stock adjustments on archived products.",
                }
            }
        }
        return { allowed: true }
    ...
    "users:update": (user, attrs) => {
        if (attrs?.userProfile) {
            ...
            if (!user.permissions.includes("users:update") && id !== user.id) {
                return {
                    allowed: false,
                    reason: "Unauthorized. You are only permitted to update your own profile.",
                }
            }
        }
        return { allowed: true }
    ...
    "users:delete": (user, attrs) => {
        if (attrs?.userProfile) {
            ...
            const isLastAdmin = (attrs.userProfile as { isLastAdmin?: boolean }).isLastAdmin
            ...
            if (roleName.toLowerCase() === "admin" && isLastAdmin) {
                return {
                    allowed: false,
                    reason: "Security block. Cannot delete the last Administrator account in the system.",
                }
            }
        }
        return { allowed: true }
    ...
    "products:delete": (_user, attrs) => {
        if (attrs?.product) {
            const hasDependencies = (attrs.product as { hasDependencies?: boolean }).hasDependencies
            if (hasDependencies) {
                return {
                    allowed: false,
                    reason: "Dependency constraint. Cannot delete product with sales or adjustments history. Archive it instead.",
                }
            }
        }
        return { allowed: true }
...
export function evaluatePolicy(action: Action, user: AuthUser, attrs?: ResourceAttributes): PolicyDecision | null {
