...
export async function CustomerStatsWrapper() {
	...
	const columns: Column<CustomerStats>[] = [
		...
		{
			header: "Total Spent",
			accessor: (c) => formatCurrency(c.totalSpent),
			className: "text-right",
		},
	];
...
export async function OverviewWrapper() {
	const [profitLoss, topProducts] = await Promise.all([
		getProfitLossReport(),
		getTopSellingProducts(),
	]);
...
export async function PurchaseHistoryTableWrapper() {
	...
	const columns: Column<(typeof purchaseHistory)[0]>[] = [
		{
			header: "Date",
			accessor: (t) => format(new Date(t.date), "MMM d, yyyy"),
		},
		{
			header: "ID",
			accessor: (t) => (
		...
		{
			header: "Total",
			accessor: (t) => formatCurrency(t.total),
...
export async function SupplierStatsWrapper() {
	...
	const columns: Column<SupplierStats>[] = [
		...
		{
			header: "Transactions",
			accessor: (s) => s.transactionCount,
			className: "text-right",
		},
		{
			header: "Total Purchased",
			accessor: (s) => formatCurrency(s.totalPurchased),
			className: "text-right",
		},
	];
	...
	const stats = [
		{
			...
			value: formatCurrency(params.totalCost),
			icon: "currency" as const,
		},
		{
			label: "Total Retail Value",
			value: formatCurrency(params.totalRetail),
			icon: "trending" as const,
		},
