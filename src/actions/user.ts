...
export async function createUser(_prevState: ActionState | null, formData: FormData): Promise<ActionState> {
    ...
    try {
        ...
        await logActivity("USER_CREATE", { id: newUser.id, email: newUser.email, name: newUser.name, role: role.name })
...
export async function updateUser(_prevState: ActionState | null, formData: FormData): Promise<ActionState> {
    const session = await auth()
    if (!session?.user) {
        return { error: "Unauthorized" }
    }
    ...
    try {
        ...
        if (userToUpdate.role.name === "Admin" && role.name !== "Admin") {
            ...
            if (adminRole) {
                ...
                if (adminCount <= 1) {
                    return {
                        error: "Security block. Cannot demote the last system Administrator.",
                    }
                }
            }
        }
...
export async function deleteUser(userId: string): Promise<ActionState> {
    ...
    try {
        ...
        if (transactionCount > 0 || adjustmentCount > 0) {
            return {
                error: "Cannot delete user. This user has recorded transactions or stock adjustments in the audit trail. Deleting them would violate database integrity.",
            }
        }
...
export async function updateProfile(_prevState: ActionState | null, formData: FormData): Promise<ActionState> {
    ...
    try {
        ...
        await logActivity("PROFILE_UPDATE", { name: rawData.name, email: rawData.email })
